...
새로 추가된 `HDRPhysicalDataset` 클래스는 EXR/HDR 파일을 톤매핑 없이 그대로 로드합니다. 선형 휘도 스케일을 유지하면서 512×1024 해상도로 Lanczos 리샘플링합니다. S2R-HDR과 Laval 데이터셋을 자동으로 감지하여 적절한 전처리를 적용합니다.

주요 기능:
- `linear_hdr=True` 옵션으로 톤매핑 비활성화
- ITU-R BT.709 표준에 따른 휘도 계산 메서드
- 데이터셋 타입 자동 감지 (s2r_hdr / laval)
...
**S2R-HDR 전처리**: EXR/HDR 파일을 512×1024 Equirectangular로 리사이즈합니다. Lanczos 리샘플링을 적용하고 음수 값을 제거한 후 FP32 EXR로 저장합니다.
...
Low-Rank Adaptation(LoRA) 어댑터를 구현합니다.

Stage 2 학습에서는 Stage 1의 구조적 지식을 보존하면서 물리적 보정만 수행해야 합니다. LoRA는 원본 가중치를 동결하고 저랭크 행렬만 학습하여 이를 효율적으로 달성합니다.

`LoRALayer` 클래스는 Linear 레이어용, `LoRAConv2d` 클래스는 Conv2d 레이어용입니다. 두 클래스 모두 원본 레이어를 래핑하고, 순전파 시 원본 출력에 LoRA 출력을 더합니다.

`apply_lora_to_generator` 함수는 Generator의 특정 레이어(affine, conv)에 LoRA를 자동으로 적용합니다. rank=16으로 설정하면 전체 파라미터의 약 6%만 학습하면서도 효과적인 보정이 가능합니다.

학습 완료 후 `merge_lora_weights` 함수로 LoRA 가중치를 원본에 병합하면, 추론 시 추가 연산 없이 사용할 수 있습니다.
...
Laval 실측 데이터셋을 사용하여 물리적 휘도 정합성을 보정합니다. Stage 1 체크포인트를 로드하고 LoRA를 적용한 후, DTAM과 PU21을 활용한 물리 손실로 학습합니다.
...
CLI 옵션으로 LoRA 랭크, DTAM 파라미터, 손실 가중치 등을 조정할 수 있습니다.
...
| **LoRA** | Stage 1 구조 보존하면서 효율적인 물리 보정 | training/lora.py |
