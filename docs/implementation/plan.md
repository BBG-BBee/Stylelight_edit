...
### 1.1 목표
단일 NFoV HDR 이미지(실행 시 초점거리 입력)로부터 180° 반구형 HDR 파노라마를 생성하는 것이 목표입니다. 생성된 파노라마는 건축 조명 설계에서 사용하는 DGP(주광 눈부심 확률) 계산에 활용되므로, 물리적으로 정확한 휘도값을 출력해야 합니다.
...
새로운 `HDRPhysicalDataset` 클래스를 추가하여 EXR/HDR 파일을 FP32 정밀도로 직접 로드합니다. 이 로더는 선형 휘도 스케일을 유지하며, 음수 값 제거와 로그 도메인 리사이즈를 수행합니다. Stage 1에서는 카메라 기울기 증강(azimuth ±π, elevation ±15°, roll ±7.5°)을 적용하고, 추론 시 LFOV HDR 입력도 동일한 log-domain 다운스케일을 적용합니다.
...
| `training/s2r_adapter.py` | S2R-Adapter (2-브랜치 도메인 적응) |
