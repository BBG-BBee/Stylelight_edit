...
총 5개의 기존 파일이 수정되었고, 14개의 새로운 파일이 생성되었습니다.
...
**Laval 전처리**: Laval은 EXR 포맷으로 절대 휘도(cd/m²)가 이미 반영되어 있어 리사이즈 및 포맷 통일을 수행합니다. .hdr 포맷 입력 시에는 Radiance HDR 헤더의 EXPOSURE 값을 읽어 절대 휘도로 보정합니다. 휘도 통계(최소, 최대, 평균, 중앙값)를 수집하여 데이터셋의 특성을 파악할 수 있습니다.
...
`log_domain_resize()` 함수를 구현합니다. HDR 이미지를 로그 도메인에서 리사이즈하여 피크 휘도를 보존합니다. log(img+eps) → resize(INTER_AREA) → exp()-eps 파이프라인으로 Lanczos 링잉 아티팩트를 방지합니다.
...
### 3.3 Phase 2+: TTA 및 추론 보조

#### training/tta_augment.py

Test-Time Adaptation(TTA) 증강 모듈입니다.

추론 시점에 입력의 불확실성을 측정하기 위한 증강 클래스들을 구현합니다. ExpAug(노출 조정), WBAug(화이트밸런스), FlipAug(수평 뒤집기), PermAug(채널 순열)의 4가지 증강과 IdentityAug(기본)를 포함합니다.

#### training/coaches/my_coach.py

TTA를 통합한 학습/추론 코치 클래스입니다.

`train_with_tta()` 메서드로 TTA 기반 추론을 수행합니다. 불확실성 U(x)를 측정하여 S2R-Adapter의 scale1, scale2를 동적으로 조절합니다 (α₁ = 1 - U(x), α₂ = 1 + U(x)).
...
│   ├── tonemapping.py                [수정] 톤매핑 유틸리티
│   ├── dtam.py                       [신규] DTAM 가중치 함수
│   ├── pu21.py                       [신규] PU21 인코딩
│   ├── s2r_adapter.py                [신규] S2R-Adapter (2-브랜치 도메인 적응)
│   ├── tta_augment.py                [신규] TTA 증강 모듈
│   └── coaches/
│       └── my_coach.py               [신규] TTA 통합 코치
│
├── utils/
│   └── hdr_utils.py                  [신규] HDR 유틸리티 (log_domain_resize)
│
├── inference/
│   ├── __init__.py                   [신규]
