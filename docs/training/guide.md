...
이 단계에서는 S2R-Adapter(Sim-to-Real Adapter)를 사용합니다. Stage 1 모델의 원본 가중치는 동결하고, 3-브랜치 구조로 효율적인 도메인 적응을 수행합니다. scale1, scale2 파라미터로 도메인 유사도에 따른 비율을 동적으로 조절하여 구조적 지식을 보존하면서 물리적 보정을 수행합니다.
...
S2R-Adapter는 3-브랜치 구조를 사용합니다:
- **Transfer1 (r1=1)**: 빠른 도메인 적응
- **Transfer2 (r2=128)**: 상세 물리 특성 학습
- **scale1, scale2**: 도메인 유사도에 따른 비율 동적 조절 (기본값: 1.0)

3-브랜치 구조로 전체 파라미터의 약 7-8%만 학습합니다. 추론 시 scale 값을 조절하여 합성/실측 도메인 간 품질을 최적화할 수 있습니다.
...
2. **S2R-Adapter 적용**: Generator의 affine 레이어에 3-브랜치 어댑터(S2R-Adapter)를 삽입합니다. r1=1, r2=128로 전체 파라미터의 약 7-8%만 학습 가능하게 설정합니다.
...
9. **S2R-Adapter 파라미터 갱신**: 동결된 원본 가중치는 그대로 두고, S2R-Adapter의 3-브랜치(transfer1, transfer2)만 업데이트합니다.

**핵심 설정:**
- DTAM: ON (전이 구간 강조)
- S2R-Adapter: r1=1, r2=128 (3-브랜치 구조)
