...
Laval 데이터셋도 동일한 방식으로 전처리합니다. Laval은 EXR 포맷으로 절대 휘도(cd/m²)가 이미 반영되어 있으므로 리사이즈 및 포맷 통일만 수행됩니다. 만약 .hdr 포맷 파일이 입력되는 경우, Radiance HDR 헤더의 EXPOSURE 값을 읽어 절대 휘도 스케일로 보정합니다(EXR 입력 시에는 해당 없음).
...
```python
# 스케일 설정 (scale1: 공유 브랜치, scale2: 전송 브랜치)
set_adapter_scales(generator, scale1=0.8, scale2=1.2)
```
...
!!! warning "TTA 사용 주의"
    Laval 데이터로 파인튜닝된 모델은 TTA가 **불필요**합니다. TTA는 학습에 사용되지 않은 새로운 도메인에서 추론할 때만 선택적으로 활성화하세요.

### 5.8.1 TTA 작동 원리

1. 입력 증강: 입력에 여러 변형(노출, 화이트밸런스, 뒤집기 등) 적용
2. 각 증강된 입력으로 순전파
3. 출력들의 분산(불확실성) 계산
4. 불확실성에 따라 스케일 동적 조절:
    - `scale1 = 1 - U(x)` (불확실성 높으면 공유 브랜치 감소)
    - `scale2 = 1 + U(x)` (불확실성 높으면 전송 브랜치 증가 = 임시 파인튜닝 할 것이므로)
5. 임시 정답 생성: 현재 입력 이미지에 대한 임시 정답
6. 어댑터 임시 파인튜닝: 출력값이 가짜 정답과 비슷해지도록 역전파를 통해 전송 브랜치의 가중치 업데이트
...
| 증강 | 설명 | 용도 |
|------|------|------|
| `ExpAug` | 노출 조정 (EV ±1~2) | 조명 변화 시뮬레이션 |
| `WBAug` | 화이트 밸런스 (RGB 가중치) | 색온도 변화 시뮬레이션 |
| `PermAug` | 채널 순열 | 색상 불변성 테스트 |
| `FlipAug` | 수평/수직 뒤집기 | 공간 불변성 테스트 |
| `IdentityAug` | 원본 유지 | 기준점 |
...
상세 기술 문서는 [S2R-Adapter 기술 문서](../technical/s2r-adapter.md)를 참조하세요.
...
2. **해상도 통일**: 다양한 크기의 원본 이미지를 512×1024 Equirectangular 형식으로 변환합니다. 로그 도메인 리사이즈(log→INTER_AREA→exp)로 HDR 피크 휘도를 보존합니다.

3. **휘도 스케일 보정**: Laval 데이터셋은 EXR 포맷으로 절대 휘도(cd/m²)가 이미 반영되어 있어 별도 보정이 불필요합니다. .hdr 포맷 파일이 입력되는 경우에는 Radiance HDR 헤더의 EXPOSURE 값을 읽어 절대 휘도로 보정합니다. S2R-HDR은 상대적 스케일을 유지합니다.
...
Stage 1에서는 카메라 기울기(elevation/roll) 증강을 적용하여 다양한 촬영 각도를 시뮬레이션합니다. skylibs의 `EnvironmentMap.rotate("DCM", dcm)`으로 ERP를 실제 회전하는 방식입니다.

| 파라미터 | 의미 | 범위 |
|----------|------|------|
| azimuth | 수평 회전(yaw) | ±π (전체, ERP wrap-around라 정보 손실 없음) |
| elevation | 상하 기울기(pitch) | ±π/12 (±15°) |
| roll | 좌우 기울기 | ±π/24 (±7.5°) |

Stage 2(물리 보정)에서는 절대 휘도 캘리브레이션이 목적이므로 기하 왜곡이 방해가 됩니다. 따라서 기울기 증강은 **Stage 1에서만 활성화**합니다.
...
2. **S2R-Adapter 적용**: Generator의 affine 레이어에 2-브랜치 S2R-Adapter를 삽입합니다. r1=1, r2=128로 전체 파라미터의 약 7-8%만 학습 가능하게 설정합니다.
...
9. **S2R-Adapter 파라미터 갱신**: 동결된 원본 가중치는 그대로 두고, S2R-Adapter의 전송 브랜치(Transfer1, Transfer2)만 업데이트합니다.

**핵심 설정:**
- DTAM: ON (전이 구간 강조)
- S2R-Adapter: r1=1, r2=128 (2-브랜치 구조)
- scale1, scale2: 도메인 유사도에 따른 비율 조절
- 일관성 손실 가중치: 0.5
