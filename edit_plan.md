...
Stage 2에서는 Stage 1의 구조적 지식을 보존하기 위해 LoRA(Low-Rank Adaptation)를 사용합니다. 원본 가중치는 동결하고, 소수의 추가 파라미터만 학습하여 효율적으로 물리적 보정을 수행합니다.
...
Stage 1에서 학습된 구조적 지식을 보존하면서 물리적 보정을 수행하기 위해 LoRA를 사용합니다. 원본 레이어의 가중치는 동결하고, 저랭크 행렬(A, B)만 학습합니다.

rank=16으로 설정하면 전체 파라미터의 약 6%만 학습하면서도 효과적인 보정이 가능합니다.
